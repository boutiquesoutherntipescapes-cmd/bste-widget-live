...
function nightsBetween(startDate, endDate) {
  const a = new Date(String(startDate) + 'T00:00:00Z');
  const b = new Date(String(endDate) + 'T00:00:00Z');
  return Math.max(0, Math.round((b - a) / 86400000));
}

export async function loadBeds24DashboardBookings(propertySlug, options = {}) {
  const roomId = await getBeds24RoomId(propertySlug);
  const params = new URLSearchParams({
    roomId: String(roomId),
    includeInvoiceItems: 'true'
  ...
  const includePastDays = Math.max(0, Number(options.includePastDays || 0));
  const earliest = addDays(today, -includePastDays);

  return rows
    .filter(row => !['cancelled', 'cancelledbyguest', 'cancelledbyhost'].includes(String(row.status || '').toLowerCase()))
    .filter(row => String(row.departure || '') >= earliest)
    .map(row => {
      const adults = Number(row.numAdult || row.adults || 0);
      const children = Number(row.numChild || row.children || 0);
      const rawPrice = Number(row.price || 0);
      const invoiceTotal = Array.isArray(row.invoiceItems)
        ? row.invoiceItems.reduce((sum, item) => sum + Number(item.amount || item.price || 0), 0)
        : 0;
      ...
      return {
        ...
        nights: nightsBetween(row.arrival, row.departure),
        status: String(row.status || ''),
        channel: sourceLabel(row.channel),
        raw_channel: String(row.channel || ''),
        guest_name: [row.firstName, row.lastName].filter(Boolean).join(' ').trim() || 'Guest',
        email: String(row.email || ''),
        mobile: String(row.mobile || ''),
        adults,
        children,
        guests: adults + children,
        price: rawPrice > 0 ? rawPrice : invoiceTotal,
