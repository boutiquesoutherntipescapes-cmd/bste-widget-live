...
async function fetchIcsCached(url, fromURLCompat) {
  ...
  if (entry && (now - entry.t) < ICS_TTL_MS) {
    return entry.data;
  }
...
async function loadFeedsForProperty(prop) {
  ...
  if (!urls.length) {
    return { feeds_ok: 0, busyNights: new Set() };
  }
  ...
  for (const r of results) {
    ...
    for (const ev of r.events) {
      for (const n of (utils.nightsBetween?.(ev.start, ev.end) || [])) {
        busy.add(n);
      }
    }
  }
...
export default async function handler(req, res) {
  try {
    ...
    const failedFeeds = results
      .filter(r => r && r._skipped && r.reason === 'feeds_failed')
